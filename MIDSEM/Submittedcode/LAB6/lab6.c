...
void dijkstras()
{
  int temp_n,current,edgenode,weight,currentnode_distance;
  struct Adj *temp;
  ...
  while(1)
  {
    ...
    while(temp!=NULL)
    {
        ...
        weight=temp->weight;
        if(vertex[edgenode]->distance>(weight) && vertex[edgenode]->visited!=1)
        {
          vertex[edgenode]->distance=weight;
          vertex[edgenode]->prev=current;
          decrease_key(vertex[edgenode]->heap_position,weight);
        }
        temp=temp->next;
    }
...
void print()
{
  ...
  {
    if((vertex[graph->total_vertices]->prev)!=-1)
    {
      printf("%d --%d \t %d\n",graph->total_vertices,vertex[graph->total_vertices]->prev,vertex[graph->total_vertices]->distance);
      total=total+vertex[graph->total_vertices]->distance;
    }
    graph->total_vertices=graph->total_vertices-1;
  }
